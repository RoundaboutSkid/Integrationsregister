...
import { z } from "zod";
import { RivtaDoc } from "../lib/schemas/zod";

export async function ingestRivtaSync(doc: z.infer<typeof RivtaDoc>) {
  ...
  await runTx(async (tx) => {
    ...
    await tx.run(`MERGE (i:Integration {id:$id}) SET i.title=$title, i.pattern=$pattern, i.interaction_type=$itype`, { id: integ.id, title: integ.title, pattern: integ.pattern, itype: integ.interaction_type });
    ...
    for (const iface of doc.interfaces) {
      ...
      const secId = `RIVTA:${iface.security.encryption}|${iface.security.server_identification}|${iface.security.access_control}`;
      ...
      if (iface.rivta?.contract) {
        await tx.run(`MERGE (c:RivtaServiceContract {id:$id}) SET c.name=$id`, { id: iface.rivta.contract });
        await tx.run(`MATCH (f:Interface {id:$fid}), (c:RivtaServiceContract {id:$cid}) MERGE (f)-[:USES_CONTRACT]->(c)`, { fid: iface.id, cid: iface.rivta.contract });
      }
